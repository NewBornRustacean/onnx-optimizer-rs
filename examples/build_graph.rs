...
use onnx_optimizer_rs::{Graph, load_model, GraphView};
use std::fs;
use std::path::Path;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(graph_proto) = &model_proto.graph {
        ...
        println!("   Initializers: {} constant tensors", graph_proto.initializer.len());
    }
        ...
        for &input_id in graph.graph_inputs() {
            if let Some(tensor) = graph.tensor(input_id) {
                println!("     - {}: {:?} {:?}", 
                    ...
                    tensor.dtype,
                    tensor.shape.as_ref().map(|s| format!("{:?}", s)).unwrap_or("unknown shape".to_string())
                );
            }
        }
    }
