...
impl GraphEdit for Graph {
    ...
    fn replace_value(&mut self, old_value: ValueId, new_value: ValueId) {
        ...
        if consumer_indices.is_empty() {
            Self::replace_in_graph_boundaries(
                &mut self.graph_input_values,
                &mut self.graph_output_values,
                old_value,
                new_value,
            );
            return;
...
mod tests {
    ...
    fn test_replace_value_multiple_consumers() {
        ...
        let node2 =
            create_test_node_with_io(OpKind::Sigmoid, vec![value_ids[0]], vec![value_ids[2]]);
